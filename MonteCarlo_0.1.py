...
pos = np.zeros((Np, 2))
NperL = int(np.ceil(np.sqrt(Np)))  # Number of particles per row
a = L / NperL                      # Lattice spacing
...
# -----------------------------
# Assign particle types: 0 for A, 1 for B
# -----------------------------
...
def lj_potential(r2, epsilon):
    """Calculate Lennard-Jones potential energy given squared distance and epsilon."""
    if r2 < (2.5 * sigma)**2:
        r2_inv = 1.0 / r2
        r6_inv = r2_inv**3
        r12_inv = r6_inv**2
...
def calculate_rdf(pos, binsize, L, nbins):
...
# -----------------------------
# Plot potential energy vs MC steps
# -----------------------------
...
# -----------------------------
# Animation with particle type colors
# -----------------------------
